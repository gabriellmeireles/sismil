<div>
    <div class="page-header mb-4">
        <div class="row align-items-center">
            ...
            <div class="col-auto ms-auto d-print-none">
                <div class="d-flex">
                    <a href="#" class="btn btn-green" data-bs-toggle="modal" data-bs-target="#create_state-modal">
                    ...
                    <div class="table-responsive">
                        <table class="table table-vcenter card-table table-striped" id="table">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    <td class="text-muted">{{ $state->id }}</td>
                                    <td>{{ $state->full_name }}</td>
                                    <td>{{ $state->short_name}}</td>
                                    <td>{{ $state->created_at->format('j M Y, h:i:s')}}</td>
                                    <td>{{ $state->updated_at->format('j M Y, h:i:s')}}</td>
                                    <td>
                                        {!! ($state->status == 0 ) ? "<span class='p-2 badge bg-orange-lt'>Inativo</span>"
                                        : "<span class='p-2 badge bg-green-lt'>Ativo</span>" !!}
                                    </td>
                                    <td>
                                        <a wire:click.prevent='edit({{ $state }})'
                                            class="btn btn-outline-lime btn-sm">Editar</a>

                                        <a wire:click.prevent='delete({{ $state }})'
    ...
    @include('livewire.admin.state.state-create')
    @include('livewire.admin.state.state-edit')
    @include('livewire.admin.state.state-disable')